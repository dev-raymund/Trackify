  ...
  try {
    ...
    if (result.status === 200) {
      ...
      if(result.type == 'income') {
        ...
        document.getElementById('remaining-budget').textContent = `₱ ${progress.toFixed(2)}`;
...
async function fetchDashboardDetails() {
  ...
  try {
    ...
    if (result.status === 200) {
      dashboardTotalIncome.textContent = `₱ ${result.total_income.toFixed(2)}`;
      dashboardTotalExpenses.textContent = `₱ ${result.total_expense.toFixed(2)}`;
    }
  } catch (error) {
...
async function fetchActivityDetails() {
  ...
  try {
    ...
    if (result.status === 200) {
      activityTotalExpenses.textContent = `₱ ${result.total_expense.toFixed(2)}`;
      activityTotalIncome.textContent = `₱ ${result.total_income.toFixed(2)}`;
    }
  } catch (error) {
      console.log("Error fetching activities:", error);
  }
...
async function fetchActivities() {
  try {
    ...
    if (result.status === 200) {
      ...
      balanceDisplay.textContent = `₱ ${result.current_balance.toFixed(2)}`;
      incomeDisplay.textContent = `₱ ${result.total_income.toFixed(2)}`;
      expenseDisplay.textContent = `₱ ${result.total_expense.toFixed(2)}`;
  ...
  li.innerHTML = `
        <div class="flex w-100 jc-space-between">
          <div class="w-70">
            ...
            <h5 class="${activity.type === "expense" ? "red" : "green"}">${activity.type === "income" ? "+" : "-"}₱ ${activity.amount}</h5>
          </div>
...
async function fetchBudgets() {
  ...
  if (result.status === 200 && result.budgets.length > 0) {
    ...
    totalBudgets.textContent = `₱ ${totalBudgetAmount.toFixed(2)}`;
...
document.getElementById('setSavingsGoal').addEventListener('submit', async function (e) {
  ...
  try {
    const response = await fetch(`${server}/set_savings_goal.php`, {
      method: 'POST',
      body: new URLSearchParams({ user_id: user_id, goal_name: goalName, target_amount: targetAmount, end_date: goalEndDate }),
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    });
